...
public class Exercise2b {
  ...
  /**
   * Finds the index of the first element in an array that is smaller than the
   * element before it.
   *
   * @example // returns 3 stepDown([10, 12, 15, 14, 16, 20])
   * @example // returns -1 (no element is smaller than its predecessor)
   *          stepDown([5, 10, 15, 20])
   * @param {int[]} numbers - An array of numbers to be examined.
   * @returns {int} The index of the first element that is smaller than its
   *          predecessor. Returns -1
